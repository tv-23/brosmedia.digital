 const container = document.getElementById('designContainer');
  ...
  const points = [
    { x: 0, y: corePoints[0].y },
    ...corePoints,
    { x: 100, y: corePoints[corePoints.length - 1].y }
  ];
  ...
  function positionElements() {
    ...
    // Calculate exact distance for each corner
    cornerDistances = corePoints.map(corner => {
      let distance = 0;
      for (let seg of pathSegments) {
        const segEndX = seg.p2.x;
        const segEndY = seg.p2.y;
        if (
          (corner.x / 100) * w === segEndX &&
          (corner.y / 100) * h === segEndY
        ) {
          distance += seg.length;
          break;
        } else {
  ...
  function moveCubes() {
    ...
    const sectionHeight = rect.height * 300;
    const viewportHeight = window.innerHeight;

    // Clamp scroll progress between 0 and 1 relative to section
    let scrollProgress = ((viewportHeight - rect.top) / (viewportHeight + sectionHeight))*500;
